...
abstract contract KaliDAOtoken {
    ...
    function transfer(address to, uint256 amount) public payable virtual notPaused returns (bool) {
    ...
    function transferFrom(address from, address to, uint256 amount) public payable virtual notPaused returns (bool) {
...
contract KaliDAO is KaliDAOtoken, Multicall, NFThelper, ReentrancyGuard {
    ...
    function cancelProposal(uint256 proposal) public payable virtual nonReentrant {
        Proposal storage prop = proposals[proposal];
    ...
    function sponsorProposal(uint256 proposal) public payable virtual nonReentrant {
        Proposal storage prop = proposals[proposal];
    ...
    function vote(uint256 proposal, bool approve) public payable virtual nonReentrant {
    ...
    function mintShares(address to, uint256 amount) public payable virtual onlyExtension {
        _mint(to, amount);
    }

    function burnShares(address from, uint256 amount) public payable virtual onlyExtension {
        _burn(from, amount);
    }
}
