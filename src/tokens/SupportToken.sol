...
contract SupportToken is pERC1155 {
    ...
    function buildSvgProgress(QuestDetail memory qd) public pure returns (string memory) {
        return string.concat(
            SVG._text(
                string.concat(
                    SVG._prop("x", "20"), SVG._prop("y", "145"), SVG._prop("font-size", "12"), SVG._prop("fill", "#000")
                ),
            ...
            SVG._rect(
                string.concat(
                    ...
                    SVG._prop("x", "20"),
                    SVG._prop("y", "165"),
                    SVG._prop("width", SVG._uint2str(uint256(250) * uint256(qd.progress) / uint256(100))),
                    SVG._prop("height", SVG._uint2str(20))
                ),
                SVG.NULL
            )
        ...
        returns (string memory)
    {
        ...
        return string.concat(
            ...
            SVG._text(
                string.concat(
                    SVG._prop("x", "20"), SVG._prop("y", "40"), SVG._prop("font-size", "20"), SVG._prop("fill", "#000")
                ),
                string.concat("Mission #", SVG._uint2str(missionId))
            ),
            SVG._text(
                string.concat(
                    SVG._prop("x", "20"), SVG._prop("y", "220"), SVG._prop("font-size", "12"), SVG._prop("fill", "#000")
                ),
                string.concat("# of steps: ", SVG._uint2str(IMissions(missions).getMissionTaskCount(missionId)))
            ),
            SVG._text(
                string.concat(
                    SVG._prop("x", "20"), SVG._prop("y", "240"), SVG._prop("font-size", "12"), SVG._prop("fill", "#000")
                ),
                string.concat("Cooldown: ", SVG._uint2str(cd))
            ),
            SVG._text(
                string.concat(
                    SVG._prop("x", "20"), SVG._prop("y", "280"), SVG._prop("font-size", "12"), SVG._prop("fill", "#000")
                ),
                string.concat("Review required: ", toReview ? unicode"🧑‍🏫" : unicode"🙅")
            ),
            SVG._text(
                string.concat(
                    SVG._prop("x", "20"), SVG._prop("y", "260"), SVG._prop("font-size", "12"), SVG._prop("fill", "#000")
                ),
